...
func v2RoutingProfileOptions(profiles []routingprefs.Profile) []string {
	out := []string{"Global"}
...
func v2RoutingEditorPage(u *v2DesktopUI, gtx layout.Context) layout.Dimensions {
	...
	sort.Slice(targets, func(i, j int) bool {
		if targets[i].ServerID == targets[j].ServerID { return targets[i].ToolName < targets[j].ToolName }
	...
	for v2RoutingEditorState.ruleProfileNext.Clicked(gtx) {
		...
		if v2RoutingEditorState.ruleProfileIndex > len(prefs.Profiles) { v2RoutingEditorState.ruleProfileIndex = 0 }
	}
	for v2RoutingEditorState.specificityNext.Clicked(gtx) {
		switch v2RoutingEditorState.specificity {
		case routingprefs.SpecificityServer: v2RoutingEditorState.specificity = routingprefs.SpecificityToolSet
		case routingprefs.SpecificityToolSet: v2RoutingEditorState.specificity = routingprefs.SpecificityConditionalTool
		default: v2RoutingEditorState.specificity = routingprefs.SpecificityServer
		}
	}
	for v2RoutingEditorState.addRule.Clicked(gtx) {
		...
		for _, target := range targets {
			...
			if choice.preferred.Value { spec.Preferred = append(spec.Preferred, value) }
			if choice.deprioritized.Value { spec.Deprioritized = append(spec.Deprioritized, value) }
		}
	...
	profileOptions := v2RoutingProfileOptions(prefs.Profiles)
	selectedProfileName := profileOptions[v2RoutingEditorState.ruleProfileIndex]
				...
				return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
					layout.Rigid(card(func(gtx layout.Context) layout.Dimensions {
						return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
							...
							layout.Rigid(func(gtx layout.Context) layout.Dimensions {
								if manager.Routing.DefaultProfile == "" { return layout.Inset{Top: unit.Dp(10)}.Layout(gtx, mutedCaption(u.th, "Default profile: global")) }
								return layout.Inset{Top: unit.Dp(10)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
									return layout.Flex{Alignment: layout.Middle}.Layout(gtx,
										layout.Flexed(1, mutedCaption(u.th, "Default profile: "+manager.Routing.DefaultProfile)),
							...
							layout.Rigid(func(gtx layout.Context) layout.Dimensions {
								...
								for _, profile := range prefs.Profiles {
									p := profile
									actions := v2RoutingEditorState.profileActions[p.ID]
									if actions == nil { actions = &v2RoutingProfileActions{}; v2RoutingEditorState.profileActions[p.ID] = actions }
									for actions.makeDefault.Clicked(gtx) {
										id := p.ID
										u.async("setting default routing profile", func() error { cfg := u.core.ManagerConfig(); cfg.Routing.DefaultProfile = id; return u.core.SaveManager(context.Background(), cfg) })
									}
									for actions.remove.Clicked(gtx) {
										id, expected := p.ID, prefs.PreferenceRevision
										u.async("deleting routing profile", func() error { _, err := u.core.DeleteRoutingProfile(context.Background(), expected, id); return err })
									}
									children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
										return layout.Inset{Top: unit.Dp(8)}.Layout(gtx, compactCard(func(gtx layout.Context) layout.Dimensions {
											return layout.Flex{Alignment: layout.Middle}.Layout(gtx,
												layout.Flexed(1, func(gtx layout.Context) layout.Dimensions { return mutedCaption(u.th, p.Name+" — "+p.Description)(gtx) }),
					...
					layout.Rigid(card(func(gtx layout.Context) layout.Dimensions {
						children := []layout.FlexChild{
							...
							layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Inset{Top: unit.Dp(7)}.Layout(gtx, secondaryButton(u.th, &v2RoutingEditorState.ruleProfileNext, "Profile: "+selectedProfileName)) }),
							...
							layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Inset{Top: unit.Dp(7)}.Layout(gtx, editorSurface(u.th, &v2RoutingEditorState.subject, "Subject key (server/tool-set/tool key)")) }),
						}
						if v2RoutingEditorState.specificity == routingprefs.SpecificityConditionalTool {
							children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Inset{Top: unit.Dp(7)}.Layout(gtx, editorSurface(u.th, &v2RoutingEditorState.condition, "Condition")) }))
						}
						children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
							if len(targets) == 0 { return layout.Inset{Top: unit.Dp(10)}.Layout(gtx, mutedCaption(u.th, "Build an index first to make routing targets available.")) }
							rows := []layout.FlexChild{layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Inset{Top: unit.Dp(10)}.Layout(gtx, mutedCaption(u.th, "Choose preferred/deprioritized authoritative targets:")) })}
							for _, target := range targets {
								t := target
								key := v2RoutingTargetKey(t)
								choice := v2RoutingEditorState.targetChoices[key]
								if choice == nil { choice = &v2RoutingTargetChoice{}; v2RoutingEditorState.targetChoices[key] = choice }
								if choice.preferred.Value { choice.deprioritized.Value = false }
								if choice.deprioritized.Value { choice.preferred.Value = false }
								rows = append(rows, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
									return layout.Flex{Alignment: layout.Middle}.Layout(gtx,
										layout.Flexed(1, mutedCaption(u.th, t.ServerID+" / "+t.ToolName)),
						...
						}), layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Inset{Top: unit.Dp(10)}.Layout(gtx, primaryButton(u.th, &v2RoutingEditorState.addRule, "Save Preference")) }))
						return layout.Flex{Axis: layout.Vertical}.Layout(gtx, children...)
					})(gtx)),
					layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Spacer{Height: unit.Dp(12)}.Layout(gtx) }),
					layout.Rigid(card(func(gtx layout.Context) layout.Dimensions {
						children := []layout.FlexChild{layout.Rigid(sectionTitle(u.th, fmt.Sprintf("Preferences (%d)", len(prefs.Rules))))}
						for _, rule := range prefs.Rules {
							r := rule
							actions := v2RoutingEditorState.ruleActions[r.ID]
							if actions == nil { actions = &v2RoutingRuleActions{}; v2RoutingEditorState.ruleActions[r.ID] = actions }
							for actions.confirm.Clicked(gtx) {
								id, expected := r.ID, prefs.PreferenceRevision
								u.async("confirming routing preference", func() error { _, err := u.core.ConfirmRoutingRule(context.Background(), expected, id); return err })
							}
							for actions.remove.Clicked(gtx) {
								id, expected := r.ID, prefs.PreferenceRevision
								u.async("deleting routing preference", func() error { _, err := u.core.DeleteRoutingRule(context.Background(), expected, id); return err })
							}
							children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
								return layout.Inset{Top: unit.Dp(8)}.Layout(gtx, compactCard(func(gtx layout.Context) layout.Dimensions {
									buttons := []layout.FlexChild{
										layout.Flexed(1, func(gtx layout.Context) layout.Dimensions { return mutedCaption(u.th, fmt.Sprintf("%s · %s · %s", r.Spec.SubjectKey, r.Spec.Specificity, r.ReviewState))(gtx) }),
									}
									if r.ReviewState == routingprefs.ReviewNeedsReview {
										buttons = append(buttons, layout.Rigid(secondaryButton(u.th, &actions.confirm, "Confirm")), layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Spacer{Width: unit.Dp(6)}.Layout(gtx) }))
									}
