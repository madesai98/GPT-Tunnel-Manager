...
type v2RoutingExplorerCounts struct{ New, Agent, Review, Ready int }
...
func v2ExplorerAllMembers(targets []coreapp.V2RoutingTarget) map[string]bool {
	members := make(map[string]bool, len(targets))
	for _, target := range targets {
		members[v2RoutingTargetKey(target)] = true
	}
	return members
}
...
func v2ExplorerServerGroups(targets []coreapp.V2RoutingTarget, names map[string]string) []v2RoutingExplorerGroup {
	...
	for _, target := range targets {
		if members[target.ServerID] == nil {
			members[target.ServerID] = map[string]bool{}
		}
		members[target.ServerID][v2RoutingTargetKey(target)] = true
	}
	...
	groups := []v2RoutingExplorerGroup{{Key: "all", Title: "All tools", Members: v2ExplorerAllMembers(targets)}}
	for _, id := range ids {
		title := names[id]
		if title == "" {
			title = id
...
func v2ExplorerCapabilityGroups(targets []coreapp.V2RoutingTarget, hierarchy enrichment.CapabilityHierarchy) []v2RoutingExplorerGroup {
	...
	appendTree = func(id string, depth int) {
		...
		visited[id] = true
		groups = append(groups, v2RoutingExplorerGroup{Key: "cap:" + id, Title: capability.Name, Description: capability.Description, Depth: depth, Members: collect(id, map[string]bool{})})
		for _, child := range children[id] {
...
func v2ExplorerSelectedGroup(groups []v2RoutingExplorerGroup) v2RoutingExplorerGroup {
	...
	return v2RoutingExplorerGroup{Key: "all", Title: "All tools", Members: map[string]bool{}}
}
...
func v2ExplorerFilteredTargets(targets []coreapp.V2RoutingTarget, group v2RoutingExplorerGroup, names map[string]string, states map[string]v2RouteToolState, query string, attentionOnly bool) []coreapp.V2RoutingTarget {
	...
	for _, target := range targets {
		...
		if query != "" {
			...
			haystack := strings.ToLower(target.ToolName + " " + target.ServerID + " " + serverName + " " + state.agent + " " + state.preference)
...
func v2ExplorerSelectionFallback(filtered []coreapp.V2RoutingTarget, group v2RoutingExplorerGroup) {
	...
	switch {
	case strings.HasPrefix(group.Key, "cap:"), strings.HasPrefix(group.Key, "server:"):
...
func v2ExplorerToolbar(u *v2DesktopUI, gtx layout.Context, hierarchyUsable bool, counts v2RoutingExplorerCounts) layout.Dimensions {
	...
	return compactCard(func(gtx layout.Context) layout.Dimensions {
		...
		if gtx.Constraints.Max.X < gtx.Dp(unit.Dp(760)) {
			rows = append(rows,
				layout.Rigid(editorSurface(u.th, &v2RouteExplorer.search, "Search tools or servers…")),
				layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Inset{Top: unit.Dp(8)}.Layout(gtx, controls) }),
			)
		} else {
...
func v2ExplorerGroupPane(u *v2DesktopUI, groups []v2RoutingExplorerGroup) layout.Widget {
	return compactCard(func(gtx layout.Context) layout.Dimensions {
		...
		return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
			...
			layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Inset{Top: unit.Dp(3), Bottom: unit.Dp(8)}.Layout(gtx, faintCaption(u.th, "Select a group to filter tools.")) }),
			layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
				return v2RouteExplorer.groups.Layout(gtx, len(groups), func(gtx layout.Context, index int) layout.Dimensions {
					...
					bg := uiSurface
					if v2RouteExplorer.selectedGroup == group.Key {
						bg = uiAccentSoft
					}
					return layout.Inset{Bottom: unit.Dp(5)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
						return click.Layout(gtx, fillSurface(bg, unit.Dp(8), layout.Inset{Top: unit.Dp(8), Bottom: unit.Dp(8), Left: unit.Dp(9 + float32(group.Depth)*13), Right: unit.Dp(9)}, func(gtx layout.Context) layout.Dimensions {
							...
							return layout.Flex{Alignment: layout.Middle}.Layout(gtx,
								...
								layout.Rigid(pill(u.th, fmt.Sprintf("%d", len(group.Members)), uiSurfaceRaised, uiMuted)),
							)
						}))
					})
				})
...
func v2ExplorerToolPane(u *v2DesktopUI, filtered []coreapp.V2RoutingTarget, total int, names map[string]string, states map[string]v2RouteToolState, ready bool) layout.Widget {
	return compactCard(func(gtx layout.Context) layout.Dimensions {
		return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
			...
			layout.Rigid(func(gtx layout.Context) layout.Dimensions { return layout.Inset{Top: unit.Dp(3), Bottom: unit.Dp(8)}.Layout(gtx, mutedCaption(u.th, fmt.Sprintf("%d shown · %d live", len(filtered), total))) }),
			layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
				if len(filtered) == 0 {
				...
				return v2RouteExplorer.tools.Layout(gtx, len(filtered), func(gtx layout.Context, index int) layout.Dimensions {
					...
					bg := uiSurface
					if v2RouteWorkspace.selected == "tool:"+key {
						bg = uiAccentSoft
					}
					state := states[key]
					statusText := v2WorkspaceToolLabel(state, ready && target.AssumptionFingerprint != "")
					...
					return layout.Inset{Bottom: unit.Dp(5)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
						return click.Layout(gtx, fillSurface(bg, unit.Dp(8), layout.Inset{Top: unit.Dp(9), Bottom: unit.Dp(9), Left: unit.Dp(10), Right: unit.Dp(10)}, func(gtx layout.Context) layout.Dimensions {
							...
							stateLabel := material.Caption(u.th, statusText)
							stateLabel.Color = statusFG
							return layout.Flex{Alignment: layout.Middle}.Layout(gtx,
								layout.Flexed(1, func(gtx layout.Context) layout.Dimensions { return layout.Flex{Axis: layout.Vertical}.Layout(gtx, layout.Rigid(name.Layout), layout.Rigid(server.Layout)) }),
...
func v2WorkspaceCapabilityOverviewGraph(targets []coreapp.V2RoutingTarget, groups []v2RoutingExplorerGroup, hierarchy enrichment.CapabilityHierarchy, status indexing.Status) ([]v2RouteGraphNode, []v2RouteGraphEdge, image.Rectangle) {
	...
	for d := range layers {
		...
		widest = max(widest, float32(len(layers[d]))*nodeW+float32(max(0, len(layers[d])-1))*itemGap+70)
	}
...
func v2WorkspaceServerOverviewGraph(targets []coreapp.V2RoutingTarget, names map[string]string, states map[string]v2RouteToolState, status indexing.Status) ([]v2RouteGraphNode, []v2RouteGraphEdge, image.Rectangle) {
	...
	for i, id := range ids {
		...
		if attention > 0 {
			statusText = fmt.Sprintf("%d tools · %d attention", len(byServer[id]), attention)
		}
	...
	return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
		layout.Rigid(compactCard(func(gtx layout.Context) layout.Dimensions {
			return layout.Flex{Alignment: layout.Middle}.Layout(gtx,
				...
				layout.Flexed(1, func(gtx layout.Context) layout.Dimensions { return layout.E.Layout(gtx, faintCaption(u.th, "Overview only · drag to pan · wheel to zoom")) }),
			)
		})),
