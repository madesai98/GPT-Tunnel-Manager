...
When ChatGPT is about to use a participating Developer Plugin, the Lifecycle Skill reads `GTM_SERVER_ID`, queries the Manager MCP with that ID, and follows the lifecycle behavior appropriate to the Server Entry's reported mode and state before proceeding to the target plugin. The separately installed Lifecycle Skill remains generic and contains no server-specific names, IDs, or registry.
