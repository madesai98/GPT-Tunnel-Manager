...
func (r *Redactor) Register(v []byte) {
	...
	for _, existing := range r.values { if existing == s { return } }
...
func (r *Redactor) String(s string) string {
	...
	for _, v := range r.values { s = strings.ReplaceAll(s,v,"[REDACTED]") }
...
func (r *Ring) Add(e Event){b,_:=json.Marshal(e);n:=len(b);r.mu.Lock();defer r.mu.Unlock();r.events=append(r.events,e);r.bytes+=n;r.evictLocked()}
func (r *Ring) SetMaxMB(maxMB int){if maxMB<=0{maxMB=25};r.mu.Lock();r.max=maxMB*1024*1024;r.evictLocked();r.mu.Unlock()}
func (r *Ring) evictLocked(){for r.bytes>r.max&&len(r.events)>1{old,_:=json.Marshal(r.events[0]);r.bytes-=len(old);r.events=r.events[1:]}}
...
func New(root,capture string,memoryMB int,writeDisk bool,diskMin string,maxFileMB,keep int)(*Logger,error){
	...
	if writeDisk{d,err:=newDiskSink(filepath.Join(root,"logs","manager"),parseLevel(diskMin),maxFileMB,keep);if err!=nil{return nil,err};l.disk=d}
	return l,nil
}
...
func (l *Logger) Log(level Level,source,component,msg string,fields map[string]any){
	l.mu.RLock();capture:=l.capture;disk:=l.disk;l.mu.RUnlock()
	if rank[level]<rank[capture]{return}
	e:=Event{Timestamp:time.Now().UTC(),Level:level,Source:l.redactor.String(source),Component:l.redactor.String(component),Message:l.redactor.String(msg),Fields:redactFields(l.redactor,fields)}
	l.ring.Add(e);if disk!=nil{_ = disk.write(e)}
}
...
func (l *Logger) Reconfigure(c config.LoggingConfig) error {
	var next *diskSink
	var err error
	if c.WriteToDisk { next,err=newDiskSink(filepath.Join(l.root,"logs","manager"),parseLevel(c.DiskMinimumLevel),c.MaximumFileSizeMB,c.KeepFiles);if err!=nil{return err} }
	l.mu.Lock();old:=l.disk;l.disk=next;l.capture=parseLevel(c.CaptureLevel);l.ring.SetMaxMB(c.MemoryLimitMB);l.mu.Unlock()
	if old!=nil{return old.close()}
	return nil
}

func (l *Logger) Close()error{l.mu.Lock();disk:=l.disk;l.disk=nil;l.mu.Unlock();if disk!=nil{return disk.close()};return nil}
...
func newDiskSink(dir string,min Level,maxMB,keep int)(*diskSink,error){if maxMB<=0{maxMB=10};if keep<=0{keep=5};if err:=os.MkdirAll(dir,0o700);err!=nil{return nil,err};d:=&diskSink{dir:dir,min:min,maxBytes:int64(maxMB)*1024*1024,keep:keep};return d,d.open()}
func (d *diskSink) open()error{f,err:=os.OpenFile(filepath.Join(d.dir,"manager.jsonl"),os.O_CREATE|os.O_APPEND|os.O_WRONLY,0o600);if err==nil{d.f=f};return err}
...
func (d *diskSink) rotate()error{if d.f!=nil{_ = d.f.Close();d.f=nil};_ = os.Remove(filepath.Join(d.dir,fmt.Sprintf("manager.%d.jsonl",d.keep)));for n:=d.keep-1;n>=1;n--{_ = os.Rename(filepath.Join(d.dir,fmt.Sprintf("manager.%d.jsonl",n)),filepath.Join(d.dir,fmt.Sprintf("manager.%d.jsonl",n+1)))};_ = os.Rename(filepath.Join(d.dir,"manager.jsonl"),filepath.Join(d.dir,"manager.1.jsonl"));return d.open()}
func (d *diskSink) close()error{d.mu.Lock();defer d.mu.Unlock();if d.f!=nil{err:=d.f.Sync();closeErr:=d.f.Close();d.f=nil;if err!=nil{return err};return closeErr};return nil}
