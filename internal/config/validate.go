...
func ValidateManager(c ManagerConfig) error {
	...
	if c.ManagerTunnel.RuntimeCredentialRef != "" && !strings.HasPrefix(c.ManagerTunnel.RuntimeCredentialRef, "secret://") {
		return errors.New("manager_tunnel.runtime_credential_ref must use secret://")
	}
...
func ValidateServer(e ServerEntry) error {
	...
	switch e.Mode {
	case ModeAlwaysOn, ModeManaged, ModeManual:
	default:
		return fmt.Errorf("invalid mode %q", e.Mode)
	}
	if !tunnelIDPattern.MatchString(e.Tunnel.TunnelID) {
		return errors.New("tunnel.tunnel_id must match tunnel_<32 lowercase hexadecimal chars>")
	}
	if e.Tunnel.RuntimeCredentialRef != "" && !strings.HasPrefix(e.Tunnel.RuntimeCredentialRef, "secret://") {
		return errors.New("tunnel.runtime_credential_ref must use secret://")
	}
	...
	for k, v := range e.Environment.SecretRefs {
		...
		if !strings.HasPrefix(v, "secret://") {
			return fmt.Errorf("environment secret %q must use secret://", k)
		}
	}
...
func validateTransport(t TransportConfig) error {
	...
	case TransportManagedHTTP:
		...
		if err := validateHTTPURL(t.ManagedHTTP.URL); err != nil {
			return fmt.Errorf("managed_http.url: %w", err)
		}
	...
	case TransportExternalHTTP:
		...
		if err := validateHTTPURL(t.ExternalHTTP.URL); err != nil {
			return fmt.Errorf("external_http.url: %w", err)
		}
	default:
