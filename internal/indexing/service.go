...
func (s *Service) Status(ctx context.Context) (Status, error) {
	...
	var active *catalog.Generation
	if generation, err := s.catalog.ActiveGeneration(ctx); err == nil {
		active = &generation
		status.ActiveGenerationID = generation.ID
		status.Ready = generation.RoutingStateHash == hash
	...
	if err == nil {
		...
		countsGenerationID = staging.ID
	} else if active != nil && active.RoutingStateHash == hash {
		// Ambiguity Reviews are explicitly non-blocking and remain actionable on
		// the active generation after promotion.
		countsGenerationID = active.ID
	}
	if countsGenerationID != "" {
		counts, err := s.catalog.EnrichmentBatchCounts(ctx, countsGenerationID)
		if err != nil {
			return Status{}, err
		}
...
func (s *Service) GetBatch(ctx context.Context, kind catalog.EnrichmentBatchKind, limit int) ([]catalog.EnrichmentBatch, error) {
	...
	if err != nil && kind == catalog.BatchAmbiguityReview {
		active, activeErr := s.catalog.ActiveGeneration(ctx)
		hash, ready := s.routing.CurrentRoutingStateHash()
		if activeErr == nil && ready && active.RoutingStateHash == hash {
			generation = active
			err = nil
		}
	}
	...
	case catalog.BatchToolEnrichment:
		if generation.Status != catalog.GenerationStaging {
			return nil, catalog.ErrGenerationNotStaging
		}
	...
	case catalog.BatchAmbiguityReview:
		// Generated by accepted capability reconciliation and intentionally
		// actionable on either staging or active generations.
	default:
